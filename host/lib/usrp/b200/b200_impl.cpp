    ...
    , _tick_rate(0.0) // Forces a clock initialization at startup
{
    ...
    // Search for all supported PIDs limited to specified VID if only VID specified
        ...
        vid_pid_pair_list.push_back(
            usb_device_handle::vid_pid_pair_t(vid, B210_100T_PRODUCT_ID));
        vid_pid_pair_list.push_back(
    ...
    // Search for all supported devices if neither VID nor PID specified
        ...
        vid_pid_pair_list.push_back(
            usb_device_handle::vid_pid_pair_t(B200_VENDOR_ID, B210_100T_PRODUCT_ID));
        vid_pid_pair_list.push_back(
    ...
    if (_product == B210_PLUS or _product == B210_100T)
    {
        client_settings = std::make_shared<b210_plus_ad9361_client_t>();
    }
    if (_product == B210 or _product == B200)
    {
